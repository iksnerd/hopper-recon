...
- **Breadcrumbs.** `PageHeader` (`components/recon/page-header.tsx`) wraps shadcn's `Breadcrumb` primitives. The `segments` prop accepts plain strings (rendered inert) or `{ label, href }` objects (rendered as `next/link` via `BreadcrumbLink asChild`). Always pass an `href` on non-leaf segments so users can navigate back; the last segment is auto-rendered as `BreadcrumbPage` regardless of href. The leading `HOPPER-RECON` always links to `/dashboard`. Every non-leaf segment (breadcrumb root included) needs `shrink-0` and separators need `shrink-0` too — only the leaf gets `min-w-0`/`truncate`; a segment that can shrink without truncating overruns its neighbour at narrow widths. The leaf is also the page's only `h1` (wrapped in `<h1 className="contents">` so it stays out of the flex layout) — pass a meaningful last segment, since no page has a separate visible heading.
- **Charts.** Wrap every recharts chart in shadcn's `ChartContainer` + `ChartTooltip`/`ChartTooltipContent` (see any dashboard or history chart site), not raw `ResponsiveContainer` + `Tooltip` — the two render differently and `chart-style.ts`'s tokens (`chartFill`, `CHART_TICK`, `CHART_CURSOR`) assume the shadcn wrapper. Categorical bars should cycle fills with `chartFill(i)` (`i % CHART_FILLS.length`), never clamp with `Math.min` — clamping collapses every series past the palette length onto one (often illegible) colour. A bar chart whose data can have one dominant, heavily-skewed category (e.g. subdomain categorisation) needs a `LabelList` count label — a real category can still render at sub-pixel width next to a dominant one, and the label is what keeps that row legible.
- **Cert day formatting.** Never interpolate a TLS cert's `daysLeft` directly into a string — it goes negative for an expired cert and reads as `-29d`. Use `certDaysLabel`/`certValidityLabel` from `lib/recon-display.ts`, which flip to `expired Nd ago` below zero.

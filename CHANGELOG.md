...
- **Breadcrumb collided at 390px** — `BreadcrumbList` was `flex-nowrap` but only the leaf segment had `truncate`, and separators had no `shrink-0`; a shrinking-but-not-truncating middle segment overran the next one, so "HISTORY" and the domain name overlapped on narrow viewports. Everything but the leaf is now `shrink-0`; the `HOPPER-RECON` root drops below `sm` to buy the leaf back its width.
- **Unbounded result lists locked up the tab on large scans** — `ALL SUBDOMAINS`, `MUTATION CANDIDATES`, `find_domains`, and `URLS` panels rendered every finding as a `CopyableText` (which injects its own `<style>` block per instance) with no cap; a 24,956-subdomain scan produced 24,956 duplicate `<style>` tags and made the tab and any navigation away from it hang for 60–120s. New `HostList` component adds a filter box and a 200-row cap with "show N more" across all 9 affected panels (5 dashboard, 4 history detail).
- **Chart bars invisible past the 4th series, and in light theme** — `chart-style.ts` hardcoded dark-theme hexes and clamped fill index with `Math.min` instead of cycling, so BY CATEGORY/BY SOURCE (8–9 rows) mostly rendered `#222222` on a `#191919` card, and light theme erased the bars entirely (`#f0f0f0` on a near-white card). Fills now reference `var(--chart-1..5)` and cycle with `%`; the ramp in `globals.css` is retuned so every step clears 3:1 contrast in both themes.
- **`cert -29d` on an expired certificate** — negative day counts rendered as a bare negative number wherever a cert's `daysLeft` was shown (RECENT TARGETS tiles, stat chips, the validity-bar caption). `recon-display.ts`'s `certDaysLabel` now reads `expired Nd ago`; a new `certValidityLabel` covers the "Nd remaining" caption variant. The RECENT TARGETS tile had bypassed the label helper entirely and interpolated `daysLeft` raw — the actual source of the bug.
- **Dashboard charts used a different tooltip treatment than history** — the dashboard wrapped charts in raw `ResponsiveContainer`/`Tooltip`; history already used shadcn's `ChartContainer`/`ChartTooltip`. Standardised all three dashboard chart sites on `ChartContainer`.
- **Long-tail category data left most chart rows sub-pixel** — a single dominant category (e.g. "Other" on a large permutation scan) left every other bar too thin to see even with the palette fixed. Category bar charts now render the count as a text label next to the bar via `LabelList`, so a row stays legible regardless of how many pixels its bar gets.
- **No page had a heading** — the highest heading on any page was an `h3` inside a card title; page identity lived only in the breadcrumb. The leaf breadcrumb segment (already each page's real subject) now doubles as an `h1`, wrapped with `contents` so it doesn't affect layout.
- **Unscoped-engine banner implied a Settings control that doesn't exist** — it read "Configure in Settings →", but every scope/auth value is an engine env var and the page has no controls. Reworded to "See Settings for how to lock this down →".
